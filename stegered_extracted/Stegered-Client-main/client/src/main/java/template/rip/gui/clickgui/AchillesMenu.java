...
import template.rip.gui.clickgui.mode.EchoDock;
...
public class AchillesMenu implements Renderable {
    ...
    static {
        ...
        for (Module.Category category : Module.Category.values()) {
            ...
            if (asm != null && asm.clickGuiStyle.is(AchillesSettingsModule.styleEnum.Moon)) {
                posX += 255f;
            } else if (asm != null && asm.clickGuiStyle.is(AchillesSettingsModule.styleEnum.Echo)) {
                posX += 208f;
            } else {
                posX += 205f;
            }
    ...
    public void render() {
        ...
        if (asm != null) {
            ...
            if (asm.clickGuiStyle.is(AchillesSettingsModule.styleEnum.Echo)) {
                EchoDock.render(asm);
            }
        }
        if (LegitMenu.getInstance().isOn || ConfigParent.getInstance().isOn) return;
        if (asm != null && asm.clickGuiStyle.is(AchillesSettingsModule.styleEnum.Echo)) {
            Template.moduleManager.getModules().forEach(module -> module.setSearched(true));
        } else {
    ...
    private final Theme theme = new Theme() {
        ...
        public void preRender() {
            ...
            if (Template.moduleManager.getModule(AchillesSettingsModule.class).clickGuiStyle.is(AchillesSettingsModule.styleEnum.Echo)) {
                ImGui.getStyle().setWindowRounding(6);
                ImGui.getStyle().setFrameRounding(4);
                ImGui.getStyle().setScrollbarSize(4);
            }
            if (Template.moduleManager.getModule(AchillesSettingsModule.class).clickGuiStyle.is(AchillesSettingsModule.styleEnum.Flat)) {
                ImGui.getStyle().setWindowRounding(8);
            }
