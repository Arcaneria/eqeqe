...
public class ImguiLoader {
    ...
    public static synchronized void onGlfwInit(long handle) {
        if (isInit || initializationAttempted || handle == 0L) {
            return;
        }
    ...
    public static void onFrameRender() {
        if (!isReadyForFrame() || frameInProgress) {
            return;
        }
        ...
        try {
            ...
            try {
                if (frameStarted) {
                    // ImGui.render() is legal only after this invocation
                    // successfully started a frame with ImGui.newFrame().
                    ImGui.render();
                    endFrame();
                }
            } finally {
    ...
    private static boolean isReadyForFrame() {
        // WindowMixin normally initializes ImGui. The lazy fallback handles
        // launchers/mod combinations where the first flipFrame arrives before
        // that constructor hook has completed.
        if (!isInit && !initializationAttempted) {
            MinecraftClient client = MinecraftClient.getInstance();
            if (client != null && client.getWindow() != null) {
                long handle = client.getWindow().getHandle();
                if (handle != 0L && glfwGetCurrentContext() != 0L) {
                    onGlfwInit(handle);
                }
            }
        }
        ...
        try {
            return ImGui.getIO().getFonts().isBuilt();
