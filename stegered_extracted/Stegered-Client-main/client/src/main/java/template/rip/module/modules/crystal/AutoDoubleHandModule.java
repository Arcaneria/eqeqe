...
public class AutoDoubleHandModule extends Module {
    ...
    private final BooleanSetting doubleHandAfterKill = new BooleanSetting(this, Description.of("Double hands after you kill a player"), true, "DHand After Kill");
    ...
    private void onAttack(AttackEntityEvent.Post event) {
        if (event.target instanceof OtherClientPlayerEntity victim) {
    ...
    private void onPacketReceive(PacketEvent.Receive event) {
        ...
        if (packet.getStatus() == 35 && doubleHandAfterPop.isEnabled() && packet.getEntity(mc.world) instanceof ClientPlayerEntity) {
            needToDHand = true;
        }

        if (packet.getStatus() == 3 && doubleHandAfterKill.isEnabled() && packet.getEntity(mc.world) instanceof OtherClientPlayerEntity victim && isMyKill(victim)) {
            attackedPlayers.remove(victim.getId());
            needToDHand = true;
        }
    }

    private boolean isMyKill(OtherClientPlayerEntity victim) {
        ...
        return mc.player != null && mc.player.squaredDistanceTo(victim) <= KILL_CREDIT_RADIUS * KILL_CREDIT_RADIUS;
    }
